...
export const NameMeaningWorkstation: React.FC<NameMeaningWorkstationProps> = ({
  ...
  const updateShuffleTime = () => {
    const now = new Date();
    setLastShuffleTime(now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }));
  };
          ...
          >
            <div className="rounded-xl bg-gradient-to-r from-amber-50 via-orange-50/50 to-amber-50 px-4 py-2.5 border border-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-amber-950 shadow-2xs">
              ...
              <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                ...
                {lastShuffleTime && (
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/50 border border-amber-200/50 text-[10px] text-amber-800 font-medium" title={`Last reordered at ${lastShuffleTime}`}>
                    <RefreshCw className="w-2.5 h-2.5 text-amber-500" />
                    <span>Last: {lastShuffleTime}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <AnimatePresence mode="popLayout">
            {displayMembers.map((member, index) => (
              <motion.div
                ...
                onClick={() => toggleExpand(member.id || member.sl)}
                className={`group relative bg-white rounded-2xl border p-4.5 flex flex-col justify-between overflow-hidden cursor-pointer member-card-lift ${
                  expandedId === (member.id || member.sl) 
              ...
              >
                      ...
                      >
                        <div className="pt-3.5 mt-3.5 border-t border-slate-100 space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Personal Attributes</span>
                            <div className="h-px flex-1 bg-slate-100 ml-3"></div>
                          </div>
                          
                          <div className="grid grid-cols-1 gap-2">
                            <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-50 border border-slate-100 hover:border-amber-200 transition group/item">
                              <div className="w-6 h-6 rounded-lg bg-white shadow-sm border border-slate-200 flex items-center justify-center shrink-0">
                                <Mail className="w-3 h-3 text-amber-500" />
                              </div>
                              <div className="min-w-0">
                                <p className="text-[8px] uppercase font-bold text-slate-400 leading-none mb-0.5">Email Address</p>
                                <p className="text-[10px] font-bold text-slate-700 truncate">{member.email || 'Not shared'}</p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-50 border border-slate-100 hover:border-amber-200 transition group/item">
                              ...
                              <div className="min-w-0">
                                <p className="text-[8px] uppercase font-bold text-slate-400 leading-none mb-0.5">Contact Number</p>
                                <p className="text-[10px] font-bold text-slate-700 truncate">{member.mobile || member.whatsapp || 'Not shared'}</p>
                              </div>
                            </div>
                          </div>
                          ...
                          {member.wishingMessage && (
                            <div className="bg-amber-50/40 p-3 rounded-xl border border-amber-100/60 relative overflow-hidden group/note">
                              ...
                              <p className="text-[9px] font-black text-amber-800 uppercase tracking-tighter mb-1.5 flex items-center gap-1.5">
                                <Heart className="w-3 h-3 text-rose-400 fill-rose-400" /> Birthday Note Draft
                              </p>
                              <p className="text-[10.5px] text-slate-600 font-medium leading-relaxed italic">
                                "{member.wishingMessage}"
                              </p>
                            </div>
                          )}
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2 text-xs">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className={`p-1 rounded-full transition-colors ${expandedId === (member.id || member.sl) ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400 group-hover:text-amber-500'}`}>
                      {expandedId === (member.id || member.sl) ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </div>
                    ...
                    {onOpenGenerator && (
                      <button
                        onClick={(e) => { e.stopPropagation(); onOpenGenerator(member); }}
                    ...
                    {onSendWhatsApp && (
                      <button
                        onClick={(e) => { e.stopPropagation(); onSendWhatsApp(member); }}
