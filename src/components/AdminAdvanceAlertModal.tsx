...
import { TeamMember, TwilioConfig, AdminSheetConfig } from '../types';
import {
...
interface AdminAdvanceAlertModalProps {
  ...
  onRefreshLogs?: () => void;
  onReCollectFromSheet?: () => Promise<void>;
}
...
export const AdminAdvanceAlertModal: React.FC<AdminAdvanceAlertModalProps> = ({
  ...
  adminConfig,
  onRefreshLogs,
  onReCollectFromSheet,
}) => {
  const [adminPhone, setAdminPhone] = useState<string>(() => {
    return adminConfig?.adminWhatsApp || localStorage.getItem('admin_planning_whatsapp') || '+880163529951';
  });
  const [adminEmail, setAdminEmail] = useState<string>(() => {
    return adminConfig?.adminEmail || localStorage.getItem('admin_planning_email') || 'admin.ie@kdsgroup.net';
  });
  ...
  const [isReCollecting, setIsReCollecting] = useState<boolean>(false);
  ...
  // Synchronize with live Google Sheet collected configuration when available
  useEffect(() => {
    if (adminConfig && adminConfig.adminWhatsApp && adminConfig.adminEmail) {
      // If user hasn't created a different stored override, apply Google Sheet values
      const storedPhone = localStorage.getItem('admin_planning_whatsapp');
      const storedEmail = localStorage.getItem('admin_planning_email');
      if (!storedPhone || storedPhone === '+880163529951') {
        setAdminPhone(adminConfig.adminWhatsApp);
      }
      if (!storedEmail || storedEmail === 'admin.ie@kdsgroup.net') {
        setAdminEmail(adminConfig.adminEmail);
      }
    }
  }, [adminConfig]);
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1 text-slate-800 bg-slate-50/30">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4.5 space-y-3 shadow-2xs">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                    ...
                    Auto-Collected from Google Sheet ({adminConfig?.sheetName || 'Central IE List'})
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Designated leadership notification endpoint dynamically extracted from sheet management & header metadata
                  {adminConfig?.detectedRole ? ` • Lead: ${adminConfig.detectedRole}` : ''}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      if (onReCollectFromSheet) {
                        await onReCollectFromSheet();
                      }
                      if (adminConfig) {
                        setAdminPhone(adminConfig.adminWhatsApp);
                        setAdminEmail(adminConfig.adminEmail);
                        localStorage.setItem('admin_planning_whatsapp', adminConfig.adminWhatsApp);
                        localStorage.setItem('admin_planning_email', adminConfig.adminEmail);
                      }
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 transition cursor-pointer shadow-2xs"
            ...
            {/* Quick Reset to Sheet Values button if values differ from detected */}
            {adminConfig && (adminPhone !== adminConfig.adminWhatsApp || adminEmail !== adminConfig.adminEmail) && (
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">
                <span className="text-amber-800 flex items-center gap-1">
                  <Info className="w-3.5 h-3.5 text-amber-600" />
                  Custom overrides active. Detected in sheet: <strong>{adminConfig.adminWhatsApp}</strong> & <strong>{adminConfig.adminEmail}</strong>
                </span>
                <button
                  type="button"
                  onClick={() => {
